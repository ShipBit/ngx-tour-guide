 ...
 */
@Component({
  imports: [ToObservablePipe, AsyncPipe, NgIf, ActionComponent],
  selector: 'shipbit-ngx-tour-guide',
  templateUrl: 'ngx-tour-guide.component.html',
  styleUrl: 'ngx-tour-guide.component.scss',
...
export class NgxTourGuideComponent implements OnDestroy {
   ...
   * Template of a stop
   */
  @ViewChild('stopTemplate', { static: true })
  public stopTemplate!: TemplateRef<any>;
    ...
    this.clipPath$ = reposition$.pipe(
      ...
      retryWhen((errors) =>
        errors.pipe(
          // count errors
          scan<ScanState, any>(
            (acc, error) => ({ errorCount: acc.errorCount + 1, error }),
            {
              errorCount: 0,
              error: undefined,
            }
  ...
  private openStop(stop?: TourGuideStop) {
    ...
    if (!this.overlayRef.hasAttached()) {
      this.portalRef = new TemplatePortal(
        this.stopTemplate,
  ...
  private getElement(stop?: TourGuideStop): HTMLElement | undefined {
    ...
    return (
      stop.element instanceof HTMLElement
        ? stop.element
        : document.querySelector(stop.element)
    ) as HTMLElement;
  }
   ...
   */
  private getOffset(margin?: NumericMargin): {
    top: number;
    right: number;
    bottom: number;
    left: number;
  ...
  private setupStopAction(stop: TourGuideStop) {
    ...
    observable
      .pipe(
        take(1),
        delay(stop.action.delay || 0),
        takeUntil(this.cancelStopAction$),
  ...
  private getStopAction(stop: TourGuideStop) {
    ...
    if (typeof stop.action.event === 'string') {
      ...
      if (!stopElement) {
        throw new Error(
          'Stop action is defined but stop does not provide an element'
        );
      }
