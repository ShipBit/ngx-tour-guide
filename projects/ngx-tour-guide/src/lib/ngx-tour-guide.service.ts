...
export class NgxTourGuideService {
   ...
   */
  private currentStopSubject$ = new BehaviorSubject<TourGuideStop | undefined>(
    undefined
  );
   ...
   */
  private progressSubject$ = new BehaviorSubject<TourProgress | undefined>(
    undefined
  );
  ...
  public start(tourGuide: TourGuide | string) {
    if (this.tourGuide) {
      throw new Error(
        'Another TourGuide is already active - consider calling TourGuideService.stop.'
      );
    }

    const tour =
      typeof tourGuide === 'string' ? this.registry[tourGuide] : tourGuide;
  ...
  public next() {
    if (!this.tourGuide) {
      throw new Error(
        'No active TourGuide. Use TourGuideService.start before navigating in a tour'
      );
    }
  ...
  public previous() {
    ...
    const currentIndex = this.getCurrentIndex();
    const previousIndex =
      (this.tourGuide.stops.length + currentIndex - 1) %
      this.tourGuide.stops.length;
  ...
  public executeAction(action?: keyof TourGuideActions) {
    if (!this.tourGuide || !this.currentStopSubject$.value || !action) {
      return;
    }
  ...
  public getCurrentProceedAction(): keyof TourGuideActions | undefined {
    ...
    return index === this.tourGuide.stops.length - 1
      ? 'finishTour'
      : 'nextStop';
  }
